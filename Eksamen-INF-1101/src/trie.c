...
// This code is based on the trie code from lecture 9.
char *trie_find(trie_t *trie, char *key)
{
    // Creates an iterator that iterates down the trie, starting at the root.
    node_t *iter = trie -> root; 

    // Set everything as lowercase letters
    for(int i = 0; key[i] != '\0'; i++) 
    {
        iter = iter -> children[ASCII_TO_IDX(tolower(key[i]))]; 
        // If the entire string is iterated 
        if(iter == NULL)    
        {
            return NULL;
        }
